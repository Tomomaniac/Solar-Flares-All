...
from detection_utils import (
    preprocess_image as d_preprocess_image,
    apply_colormap as d_apply_colormap,
    load_models as load_detection_models,
    predict_flare as predict_detection_flare,
)
...
# ====
# DETECTION (merged from app_detection.py)
# ====


@st.cache_resource
def load_detection_models_cached():
    """Cached loader for detection models (v1 and v3) from `detection_utils`"""
    return load_detection_models()


def display_detection_results(result, model_num, filter_num, model_version):
    """Display detection results (adapted from app_detection.py)"""
    ...
    with res_col2:
        if model_num == "v3":
            st.metric("Raw Max Brightness", f"{features.get('raw_max', 0):.1f}")
            st.metric("Raw Mean Brightness", f"{features.get('raw_mean', 0):.1f}")
        else:
            st.metric("Max Brightness", f"{features.get('max_brightness', 0):.4f}")
            st.metric("Mean Brightness", f"{features.get('mean_brightness', 0):.4f}")
    ...
    try:
        ...
        confidence = 0.0
    flare_prob = probabilities[1] * 100 if len(probabilities) > 1 else 0.0
...
def create_detection_sidebar():
    """Sidebar info for detection module (kept separate from prediction sidebar)"""
    with st.sidebar:
        st.header("Detection — Info")
        st.markdown("""
        This section runs detection models (v3 → v1) that identify flares.

        - **Model v1**: QUIET vs DURING
        - **Model v3**: (BEFORE+QUIET) vs DURING (includes raw brightness)
        """)
        st.markdown("---")
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if is_preselected:
            if st.button("🔄 Refresh Random Image", key="refresh_random_image"):
                random_image = get_random_preselected_image()
                if random_image:
                    st.session_state.preselected_image_path = random_image
    ...
    if img is not None:
        ...
        # === Detection UI (from app_detection copy) ===
        st.markdown("---")
        st.subheader("🔎 Detection (v3 → v1)")

        # Detection selectors (reuse detected_filter if available)
        det_col1, det_col2, det_col3 = st.columns(3)
        with det_col1:
            # Filter selector (auto-detected earlier)
            if detected_filter:
                default_index = 0 if detected_filter == "131 Å" else 1
        ...
        try:
            det_img_norm = d_preprocess_image(img_array if img_array is not None else np.array(Image.open(st.session_state.preselected_image_path)))
            det_preview = d_apply_colormap(det_img_norm, det_colormap)
        ...
        if st.button("🔍 Run Detection", key="run_detection_button"):
            with st.spinner("Running detection models..."):
                try:
                    models_det, feature_cols_v1_det, feature_cols_v3_det = load_detection_models_cached()
                    # call detection predict
                    det_result = predict_detection_flare(
                        img_array,
                        det_model_num,
                        det_filter_num,
                        models_det,
                    ...
                    display_detection_results(det_result, det_model_num, det_filter_num, det_model_version)
                except Exception as e:
